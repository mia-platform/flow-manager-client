    ...
    return new Promise((resolve, reject) => {
      ...
      const checkReadinessInterval = setInterval(() => {
        ...
        if (numberOfReadinessChecks > MAX_READINESS_CHECKS_AT_START) {
          clearInterval(checkReadinessInterval)
          return reject(new Error('uable to become ready in reasonable amount of time'))
        }
      })
    }, JOIN_WAIT_TIME)
  }
...
function createCommandsDispatcher(fmClient) {
  ...
  return async({ topic, message, partition }) => {
    ...
    try {
      ...
      sagaLogger.warn({ error }, 'skip command due to parsing parsing error')
    ...
    try {
      ...
      await commandFunction(sagaId, payload, eventEmitter)
    } catch (executorError) {
      sagaLogger.error(
        {
          command,
